...
>
> > « ultrathink. Lis intégralement `cahier-des-charges-que-du-kheir.md` à la racine : c'est le cahier des charges complet d'un site vitrine pour l'association **Que du Kheir**. **EXIGENCE FORTE DE CETTE VERSION : des animations ÉDITORIALES EXTRAVAGANTES et une grosse couche GSAP** (scroll narratif, texte qui se compose, images en colonnes décalées, parallaxe profonde, transitions de page) — **fortement inspirées de la référence `wearediasporas.com`** (média éditorial marron/crème, gros titres, mise en page magazine). **Pas de 3D** : tout passe par **GSAP + ScrollTrigger + SplitText + Lenis + Framer Motion**. **L'extravagance se concentre sur la VITRINE (accueil, histoire, actions, galerie) ; le DASHBOARD de transparence et les DONS restent sobres, clairs et dignes de confiance** (asso → crédibilité institutions). Lis en priorité §4 (DA + dispositif d'animation), §4-bis (référence Diasporas), §5 (assets), §6 (dashboard). **Étape 1 : exécute la procédure d'assets (§5)** (extraire logo/couleurs du site, sinon DA de repli) AVANT de coder. Construis le prototype Next.js, installe les dépendances (dont gsap + @gsap/react + lenis), vérifie `npm run dev` sans erreur **et que les animations GSAP ne cassent pas le build/hydratation**, puis donne-moi un récap (arborescence, lancement, inventaire assets, points à brancher au build réel). »
>
...
### Animations & finition — ÉDITORIAL EXTRAVAGANT (le gros changement)
> **Objectif : un site qui claque, façon magazine éditorial vivant** — pas une démo d'agence tape-à-l'œil, mais une **mise en scène riche du contenu** dans l'esprit Diasporas poussé à fond. **L'extravagance vit sur la VITRINE** (accueil, histoire, actions, galerie, blog) ; le **dashboard transparence + dons restent sobres** (cf. §6).
- **Smooth scroll** : **Lenis** (inertie douce) sur tout le site — base du ressenti premium.
- **GSAP + ScrollTrigger partout sur la vitrine** :
  - **Titres qui se composent** : **SplitText** (mot par mot / lettre par lettre), gros titres éditoriaux en capitales (comme Diasporas) qui se révèlent au scroll avec masque/clip, décalages (stagger).
  - **Sections épinglées (pin)** + **scrub** : sequences storytelling (la mission, le parcours d'une maraude) qui se déroulent au scroll.
  - **Parallaxe profonde multi-couches** : photos de terrain qui bougent à des vitesses différentes, superpositions, **horizontal scroll** ponctuel (galerie/chiffres/valeurs façon bande défilante).
  - **Reveals au masque** : images qui apparaissent par volets/wipe, lignes qui se tracent, compteurs de chiffres clés animés au scroll.
  - **Marquee** (texte défilant) pour les valeurs / le slogan / les actions.
- **Transitions de page** cinématiques (overlay/wipe couleur terre/crème entre les pages) via Framer Motion + un layout de transition.
- **Curseur** légèrement custom (discret, chaleureux — halo doux), magnétisme sur les gros CTA. (Pas de gadget froid.)
- **Micro-interactions** soignées : hovers d'images (zoom doux + légende qui monte), cartes d'actions qui s'animent, liens avec soulignés animés.
- **Hero spectaculaire mais digne** : grande image/mosaïque de terrain + titre éditorial qui se compose + léger mouvement (parallaxe/Ken Burns), accroche forte.
- **Garde-fous (IMPORTANT pour que ça MARCHE)** :
  - GSAP/ScrollTrigger/SplitText **côté client uniquement** ; utiliser **`@gsap/react` (`useGSAP`)** et **`gsap.context()`** pour un cleanup propre (éviter les erreurs Next/StrictMode et les fuites). Rafraîchir ScrollTrigger après chargement des images/polices.
  - **`prefers-reduced-motion`** : fournir des **fallbacks statiques élégants** (le contenu reste lisible sans animation) — **essentiel** vu la cible (personnes âgées, institutions).
  - **Performance** : animations sur `transform`/`opacity`, lazy, pas de jank ; le **contenu doit s'afficher même si JS lent** (SSR du texte). Tester le build.
  - **Accessibilité préservée** : l'extravagance ne casse ni la lisibilité, ni la navigation clavier, ni les contrastes.

### Imagerie — vraies images d'abord (§5)
Utiliser leurs **vraies photos de terrain** (maraudes, distributions) placées **avec cohérence** et **mises en scène** (colonnes décalées, grandes bandes, parallaxe), façon photo-reportage éditorial. **À défaut**, placeholders chaleureux dans leurs tons **clairement étiquetés “photo à fournir”** (ne pas utiliser de banque d'images trompeuse présentée comme leurs actions). **Respecter le droit à l'image** (cf. §13).
...
## 4-bis. RÉFÉRENCE D'INSPIRATION — `wearediasporas.com` (à étudier de près)

> Le client **aime** ce site et veut **rester proche de cet univers**. C'est la **boussole esthétique** de la vitrine. Reprends-en **l'esprit et le langage visuel** (sans copier-coller), en l'**animant beaucoup plus**.

Ce qu'on retient de Diasporas (média associatif loi 1901, esprit « récits/culture ») :
- **Palette chaleureuse terreuse** : **marron profond + crème/beige**, très peu de couleurs, beaucoup de **respiration**. (→ aligne notre palette §4 dessus si l'extraction de leur charte ne donne rien.)
- **Gros titres éditoriaux en CAPITALES** (« LE MÉDIA QUI DONNE VIE AUX RÉCITS », « NOTRE TRAVAIL », « ÉCRIRE NOS HISTOIRES ») — typographie forte, généreuse, magazine.
- **Mise en page éditoriale** : photos en **colonnes/portraits décalés**, blocs de texte aérés, sections nettes, **logo décliné en clair (sur fond sombre) et marron (sur fond clair)**.
- **Ton** : digne, humain, fier, « pour la culture, par la culture » — **émotion + sérieux**.
- **Structure** : Accueil / sections de contenu / Notre histoire / Notre équipe (grille de portraits) / Nous soutenir / Réseaux.

**Notre valeur ajoutée vs Diasporas (qui est un site Wix assez statique)** : on garde **ce goût éditorial** mais on le rend **vivant et spectaculaire** grâce au dispositif GSAP ci-dessus (scroll narratif, titres qui se composent, parallaxe, transitions). C'est précisément ce qui doit faire dire au client « c'est l'esprit qu'on aime, mais en **beaucoup plus fort** ».
...
- **Animation (couche lourde, vitrine)** : **`gsap`** + **`@gsap/react`** (`useGSAP`) + **ScrollTrigger** + **SplitText** (ou équivalent splitting si SplitText non dispo) + **`lenis`** (smooth scroll) + **`framer-motion`** (transitions de page, micro-interactions). Voir §4 pour le dispositif et les garde-fous (cleanup `gsap.context`, refresh après load, `prefers-reduced-motion`).
- **`framer-motion`** aussi pour les éléments simples ; respecter `prefers-reduced-motion` partout.
- **`next/image`** (images locales) ; `next/font`.
- **Séparation nette** : **vitrine** (accueil, histoire, actions, galerie, blog) = **animations extravagantes** ; **dashboard transparence + dons + formulaires** = **sobres, rapides, lisibles** (animations minimales, priorité clarté/confiance).
...
- **DA = chaleureuse, éditoriale, terreuse (esprit Diasporas), humaine et digne de confiance**, **laïque**. **Sur la vitrine, l'exécution doit être SPECTACULAIRE** (scroll narratif, typo qui se compose, parallaxe, transitions — cf. §4) ; Taste Skill **pousse l'audace éditoriale et la finition**. **Pas** de dark/néon, **pas** de 3D, **pas** de motifs cultuels. **Sur le dashboard/dons/formulaires : sobriété et clarté** priment.
- **Dials recommandés** :
  - **Vitrine** — **Design Variance : haute** (éditorial audacieux, mise en page magazine vivante) ; **Motion Intensity : haute** (GSAP, scroll narratif, parallaxe — mais digne, jamais clownesque) ; **Visual Density : basse-modérée** (gros titres qui respirent, photos qui portent).
  - **Dashboard / dons / formulaires** — **Variance basse, Motion faible, Density modérée** (lisibilité, confiance, accessibilité).
  - Preset **éditorial chaleureux audacieux** pour la vitrine ; **clair/utilitaire** pour le fonctionnel.
- Active la variante **anti-laziness / full-output** (multi-pages + dashboard + grosse couche animation).
- En cas de doute : sur la **vitrine**, oser l'effet ; sur le **fonctionnel**, **confiance + clarté + accessibilité** (cible non-Insta, personnes âgées) priment **toujours**.
...
**Réutilisables** : `Header`, `AnnouncementBar` (campagne du mois), `Footer`, `MobileActionBar`, `SmoothScroll` (wrapper Lenis), `PageTransition` (wipe terre/crème), `SplitHeading` (titre éditorial qui se compose, GSAP/SplitText), `Reveal` (masque/wipe au scroll), `Parallax` (couches multi-vitesses), `Marquee` (valeurs/slogan défilants), `HorizontalScroll` (galerie/valeurs en bande), `Cursor` (halo doux discret), `Magnetic` (gros CTA), `FramedImage`, `Divider`, `SectionTitle`, `ActionCard`, `StatCounter` (compteur animé au scroll), `DonationCallout` (→ HelloAsso), `NeedsList`, `EventCard`/`AgendaList`, `GalleryGrid` (mosaïque animée), `PostCard`, `VolunteerForm`, `TransparencyDashboard` (+ `BudgetDonut`, `AllocationBars`, `ExpensesTable`, `ProjectProgress` — **sobres**), `AuthForm`, `TestimonialSlider`, `InstaGrid` (optionnel). Vitrine **très animée** ; dashboard/formulaires **sobres**. Mobile-first, **accessibles**, **fallbacks `prefers-reduced-motion`** partout.

### 11.1 Accueil (vitrine spectaculaire, esprit Diasporas animé)
- **Hero éditorial** : grande **mosaïque/bande de photos de terrain** (parallaxe douce / Ken Burns) + **titre en CAPITALES qui se compose** (SplitText, façon « LE MÉDIA QUI… » de Diasporas → ici ex. **« L'ENTRAIDE QUI CHANGE DES VIES »** / accroche fidèle au pitch), sous-titre (entraide & lutte contre la précarité, 93 + Paris). CTA **« Faire un don »** (magnétique) + **« Découvrir nos actions »**. Entrée cinématique (overlay qui se lève).
- **Notre mission** : section au **scroll narratif** (texte qui se révèle ligne par ligne, mot-clé surligné en ocre) ; **valeurs en Marquee** (Solidarité · Entraide · Dignité · Humanité · Transparence) défilant.
- **Nos actions** : `ActionCard` en **mise en page éditoriale décalée** (colonnes, parallaxe) — Maraudes mensuelles, **Kits d'hygiène** systématiques, Fournitures scolaires, Projets à venir. Hover animé (zoom + légende). → page Actions.
- **Notre impact** : `StatCounter` (chiffres clés qui **s'incrémentent au scroll** — **à fournir/valider**, sinon « bientôt »/placeholders étiquetés ; **ne pas inventer**), éventuellement en **bande horizontale**.
- **Transparence (teaser)** : aperçu **sobre** du dashboard (répartition des dons) sur fond éditorial + « Voir notre transparence ». **Argument de confiance** — ici on calme l'animation (le contenu doit inspirer le sérieux).
- **Faire un don** : `DonationCallout` (ponctuel/récurrent → **HelloAsso**) + campagne du mois, présenté **clairement** (peu animé).
- **Agenda** : prochaines maraudes/actions (aperçu).
- **Galerie / réseaux** : **mosaïque animée** (reveals au masque, hover) + **Instagram** (@quedukheir) si activé ; possible **scroll horizontal**.
- **Bénévoles** : invitation forte (gros titre composé) → formulaire.
- **Contact / accès simple** : pensé aussi **hors réseaux** (personnes âgées) — adresse de contact claire, gros boutons, **sans dépendre des animations** (lisible même reduced-motion).
- **Transition de page** cinématique vers les autres rubriques.
...
Une app **Next.js qui tourne avec `npm run dev`**, ~13 pages navigables, **chaleureuse, éditoriale et terreuse dans l'esprit de leur référence Diasporas — mais BEAUCOUP plus vivante et spectaculaire** : **vitrine à fortes animations GSAP** (smooth scroll Lenis, titres en capitales qui se composent via SplitText, scroll narratif épinglé, parallaxe profonde multi-couches, marquees, scroll horizontal, reveals au masque, transitions de page cinématiques, curseur/halo discret, compteurs animés). **DA extraite si possible, sinon repli re-skinnable** ; **logo extrait/provisoire**. Affiche les **vraies images** (sinon placeholders étiquetés), mises en scène en photo-reportage éditorial. Fonctionnalités : **dons via HelloAsso**, **agenda**, **galerie**, **blog**, **formulaire bénévole**, et la **feature clé : espace adhérent + dashboard de TRANSPARENCE** — ce **dashboard et les dons/formulaires restent SOBRES, clairs et accessibles** (confiance institutions, cible non-Insta). **SEO/crédibilité** soignés (JSON-LD NGO, mentions légales/RGPD, RNA) pour servir la **priorité n°1 (Google for Nonprofits)**. **`prefers-reduced-motion` respecté partout** (fallbacks statiques élégants), build et hydratation **non cassés** par GSAP.

Soigne particulièrement : l'**accueil** et la page **Notre histoire / Nos actions** (effet *wow* éditorial maximal), **Faire un don** (clair) et le **dashboard de transparence** (sobre et lisible). Rendu **nettement plus impressionnant que la première version** et que leur site actuel, prêt à présenter en rendez-vous.
